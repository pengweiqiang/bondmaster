...
public class TextViewUtils {
	...
	public static SpannableString getSpannableStringColor(String str,int start,int length,String colorId){
		SpannableString ss = new SpannableString(str);

		ss.setSpan(new ForegroundColorSpan(Color.parseColor(colorId)), start, length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

		return ss;
	}
	public static SpannableString getSpannableStringColor(String str,String starts,String lengths,String colorId){
		SpannableString ss = new SpannableString(str);
		String startList[] = starts.split(",");
		String lengthList[] = lengths.split(",");
		for(int i =0 ;i<startList.length;i++){
			ss.setSpan(new ForegroundColorSpan(Color.parseColor(colorId)), Integer.valueOf(startList[i]), Integer.valueOf(lengthList[i]), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
