...
public class HeaderScrollingBehavior extends CoordinatorLayout.Behavior<RecyclerView>{
    ...
    public boolean layoutDependsOn(CoordinatorLayout parent, RecyclerView child, View dependency) {
        if (dependency != null && dependency.getId() == R.id.iv_banner) {
    ...
    private Runnable flingRunnable = new Runnable() {
        ...
        public void run() {
            if (scroller.computeScrollOffset()) {
                ...
                boolean isNowExpaned = getDependentView().getTranslationY() != 0;
                if(isNowExpaned!=isExpanded){
                    if(onChangeExpandedListener!=null){
                        onChangeExpandedListener.isChanged(isNowExpaned);
                    }
                    isExpanded = isNowExpaned;
                }
    ...
    private OnChangeExpandedListener onChangeExpandedListener;

    public void setOnChangeExpandedListener(OnChangeExpandedListener onChangeExpandedListener){
        this.onChangeExpandedListener = onChangeExpandedListener;
    }

    public interface OnChangeExpandedListener{
